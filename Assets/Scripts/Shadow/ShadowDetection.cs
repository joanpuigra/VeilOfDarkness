...
namespace Shadow
{
    public class ShadowDetection : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            if (transform.hasChanged && _canShadowUpdate)
            {
        ...
        // Create Gizmo to view shadow colliders
        private void OnDrawGizmos()
        {
            if (_shadowCollider != null && _shadowCollider.sharedMesh != null)
            {
                // Get MeshFilter
                Mesh colliderMesh = _shadowCollider.sharedMesh;

                // Recalculate Normals (create if not)
                if (colliderMesh.normals.Length == 0)
                {
                ...
                // Draw Mesh
                Gizmos.DrawWireMesh(colliderMesh, transform.position, transform.rotation);
            }
        }
    }
}
